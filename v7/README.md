# Group Chat System (V7)

A concurrent, multi-user group chat application built as a multi-process UDP server. This version implements a process-per-request model for connectionless communication.
...
- **Concurrency:** Multi-process server using `fork()` to handle each incoming datagram request in a separate child process.
...
- **Server Model:** Connectionless multi-process server.
